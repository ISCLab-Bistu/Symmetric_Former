...
class SeparableConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=0, dilation=1, bias=False):
        ...
        self.dw = torch.nn.Sequential(nn.Conv2d(
                                     ...
                                     groups=out_channels,
                                     padding=padding),
                                     # nn.LayerNorm([in_channels, 7, 7]),
                                     # nn.GELU(),
        )
        # pw-linear

        # self.pw = torch.nn.Sequential(nn.Conv2d(in_channels, out_channels, 1, 1, 0, bias=False),
...
class Mlp_ghost(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.primary_conv_i = nn.Sequential(
            ...
            # nn.GroupNorm(1, hidden_features//2),
            nn.GELU(),
        )
        ...
        # self.dw = torch.nn.Sequential(SeparableConv2d(hidden_features, hidden_features, kernel_size=3, stride=1, padding=1),
        #                               nn.ReLU(inplace=True)
        #                               )
        # self.norm = nn.GroupNorm(1, hidden_features)
    ...
    def forward(self, x, H, W):
        ...
        out = x.view(B, c, N).permute(0, 2, 1).contiguous()
        return out

    def flops(self, H, W):
        # flops = H * W * self.in_features * (self.hidden_features - self.in_features) + \
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., expand=False):
    ...
    def forward(self, x, mask=None):
        ...
        # B_, N, 3, hn, fn
        qkv = self.qkv(x).reshape(B_, N, self.num_heads, (self.head_dim * 2 + self.inv_head_dim)).permute(0, 2, 1, 3).contiguous()
        # 3, B_, hn, N, fn
        ...
        if self.Lepe:
            ...
            lepe = self.lev(v.permute(0, 1, 3, 2).contiguous().reshape(B_, self.num_heads * self.inv_head_dim, res, res)).\
                view(B_, self.inv_head_dim * self.num_heads, res * res).permute(0, 2, 1).contiguous()
        q = q * self.scale
        # W*B, 49, head, feature
        if self.acspa_use:
            q, k = self.acspa(q, k)
        ...
        if self.ori:
            ...
            res = int(np.sqrt(N))
            x_e = self.linal_trans(x.permute(0, 2, 1).contiguous().view(B_, C, res, res)).\
                view(B_, C, res * res).permute(0, 2, 1).contiguous()
