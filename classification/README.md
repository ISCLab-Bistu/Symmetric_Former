# SF for Image Classification

This folder contains the implementation of the SF for image classification.
...
|:-------:|:----------:|:-----:|:-------:|:-----:|:---------------------------------------------------------------------:|
| SFormer |  224x224   | 72.1  |  3.4M   | 0.5G  | [Pretained](https://pan.baidu.com/s/1O_WX0h6JymRy6TLvbVp-8g?pwd=1234) |
...
```bash
git clone https://github.com/ISCLab-Bistu/Symmetric_Former.git
cd Symmetric_Former/classification
```
...
To evaluate a pre-trained `SF` on ImageNet val, run:
...
To train a `SF` on ImageNet from scratch, run:
...
- When GPU memory is not enough, you can try the following suggestions:
    ...
    - Use gradient checkpointing by adding `--use-checkpoint`, e.g., it saves about 60% memory when training `SF`.
...
For example, to train `SF` with 8 GPU on a single node for 300 epochs, run:

`SF`:

```bash
python -m torch.distributed.launch --nproc_per_node 8 --master_port 12345  main.py \
--cfg configs/sf.yaml --data-path <imagenet-path> --batch-size 128 
```
