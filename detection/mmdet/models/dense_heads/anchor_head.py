...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _get_targets_single(self,
        ...
        """
        if torch.sum((gt_bboxes[:, 3]-gt_bboxes[:, 1])<=0)>0:
            # print(gt_bboxes, gt_labels)
            keep = (gt_bboxes[:, 3] - gt_bboxes[:, 1]) > 0
            gt_bboxes = gt_bboxes[keep]
            if gt_labels is not None:
                gt_labels = gt_labels[keep]
            # gt_bboxes_ignore = keep
        ...
        if len(pos_inds) > 0:
            if not self.reg_decoded_bbox:
                # if torch.sum((sampling_result.pos_bboxes[..., 3].float() - sampling_result.pos_bboxes[..., 1].float()) <= 0) > 0:
                #     # print(gt_bboxes, gt_labels)
                #     # keep = (gt_bboxes[:, 3] - gt_bboxes[:, 1]) > 0
                #     # gt_bboxes = gt_bboxes[keep]
                #     # if gt_labels is not None:
                #     #     gt_labels = gt_labels[keep]
                #     print()
                # print(sampling_result.pos_bboxes[..., 3].float() - sampling_result.pos_bboxes[..., 1].float())
