...
class BBoxHead(nn.Module):
    ...
    def _get_target_single(self, pos_bboxes, neg_bboxes, pos_gt_bboxes,
        ...
        if num_pos > 0:
            ...
            keep = pos_bboxes[..., 3]<1e8
            if not self.reg_decoded_bbox:
                if torch.sum((pos_bboxes[..., 3].float() - pos_gt_bboxes[..., 1].float()) <= 0) > 0:
                    keep = (pos_bboxes[..., 3].float() - pos_gt_bboxes[..., 1].float()) > 0
                    # num_pos -= torch.sum(keep)
                    pos_bboxes = pos_bboxes[keep]
                    pos_gt_bboxes = pos_gt_bboxes[keep]
            ...
            bbox_targets[:num_pos, :][keep] = pos_bbox_targets
            bbox_weights[:num_pos, :][keep] = 1
        if num_neg > 0:
